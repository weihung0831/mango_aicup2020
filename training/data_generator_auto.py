...
class DataGeneratorW(Sequence):
    ...
    def shuffle_it(self):
        'Updates indexes on call'
        if self.shuffle:
            # print('I am shuffling now...')
            np.random.shuffle(self.indexes)
    ...
    def get_data(self, sample):
        ...
        augmentation = np.random.uniform(0,2.2, 1)[0]
        kernelsize = np.random.choice([3,5])
        to_augment = np.random.choice([True, False])
        
        brightness = int(np.random.normal(0,25,1)[0])
        ...
        if 'brightness' in self.augment:
            if to_brighten:
                ...
                if np.random.choice([True, False]) == True:
                    train_image  = cv2.convertScaleAbs(train_image, alpha=1.0, beta=brightness)
                else:
                    train_image  = cv2.convertScaleAbs(train_image, alpha=1.0, beta=-brightness)  
...
class PredictGenerator(Sequence):
    ...
    def __init__(self,
                 ...
                 gamma_adjust = True,
                 augment = ['rotate', 'blur', 'brightness'],
                 shuffle=False, return_weights = True):
        self.image_loc = X
        self.labels = Y
        self.data = np.column_stack([X, Y])
        self.batch_size = batch_size
