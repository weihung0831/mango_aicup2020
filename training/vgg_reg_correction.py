...
class changeReg(tf.keras.callbacks.Callback):
    ...
    def on_epoch_begin(self, epoch, logs={}):
        a = tf.constant([(epoch*epoch/self.full_learning)-6], dtype = tf.float32)
...
class KerasResnetTrainer: 
    ...
    def __init__(self, name = 'vgg19_tune',
        ...
        def loss_fn(y_true, y_pred):
            return reg.regularizer*loss[0](y_true, y_pred)
            
        if self.cosine_annealing:
            cosine = CosineAnnealingScheduler(self.epochs, 1e-7, self.initial_lr)
            self.callbacks = [modelcheckpoint, cosine, reg]
        else:
        ...
        if as_SVM:
            ...
            class1 = layers.Dense(1028,activation=tf.nn.relu)(flat1)
            class1 = layers.Dense(1028,activation=tf.nn.relu)(class1)
        ...
        if opt =='SGD':
            optimizer = tf.keras.optimizers.SGD(learning_rate=self.initial_lr, momentum=0.9, nesterov=True)
        else:
            optimizer = tf.keras.optimizers.Adam(learning_rate=self.initial_lr)
        self.model.compile(optimizer, loss = loss_fn,  metrics = self.metrics)
            ...
            for i in range(self.epochs):
                ...
                if self.balanced:
                    ...
                    if i > (self.epochs/2):
                        ...
                        y_train = np.column_stack([ypred, y_train[:,-1]])
                    ...
                    if i > (self.epochs/5):
                        y_pred = self.predict_proba(X_train)
                        y_pred = np.round(y_pred)
                        y_train = np.column_stack([y_pred, y_train[:,-1]])
